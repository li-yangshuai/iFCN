...
std::vector<EncodedPhaseTile> encodePhaseMapToTiles(
    const std::map<PhaseCoord, int>& phaseMap,
    int phaseCount,
    int blockSize,
    int gridWidth,
    int gridHeight
    ...
    if (phaseMap.empty() && (gridWidth <= 0 || gridHeight <= 0)) {
        return {};
    }

    for (const auto& [coord, phase] : phaseMap) {
        requirePhaseValue(phase, phaseCount);
        gridWidth = std::max(gridWidth, static_cast<int>(coord.first) + 1);
        gridHeight = std::max(gridHeight, static_cast<int>(coord.second) + 1);
    }
    ...
    const int paddedWidth = ((gridWidth + blockSize - 1) / blockSize) * blockSize;
    const int paddedHeight = ((gridHeight + blockSize - 1) / blockSize) * blockSize;
    std::vector<EncodedPhaseTile> tiles;
    for (int tileY = 0; tileY < paddedHeight / blockSize; ++tileY) {
        for (int tileX = 0; tileX < paddedWidth / blockSize; ++tileX) {
            PhaseMatrix matrix(
                static_cast<size_t>(blockSize),
                std::vector<int>(static_cast<size_t>(blockSize), 0)
            );
            ...
            for (int row = 0; row < blockSize; ++row) {
                for (int column = 0; column < blockSize; ++column) {
                    const unsigned int x = static_cast<unsigned int>(tileX * blockSize + column);
                    const unsigned int y = static_cast<unsigned int>(tileY * blockSize + row);
                    const auto it = phaseMap.find({x, y});
                    matrix[static_cast<size_t>(row)][static_cast<size_t>(column)] =
                        it != phaseMap.end() ? it->second : 0;
                }
            }

            tiles.push_back({static_cast<unsigned int>(tileX),
                             static_cast<unsigned int>(tileY),
                             encodeMatrixToPackedHex(matrix, phaseCount, blockSize)});
        }
    }
    return tiles;
}
...
#ifdef PHASE_CODEC_STANDALONE
int main(int argc, char** argv) {
    try {
        using namespace fcngraph::phase_codec;
