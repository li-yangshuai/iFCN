...
struct EdgeNode {
    EdgeNode(uint64_t _tailidx, uint64_t _headidx) 
        : tailidx(_tailidx), headidx(_headidx), headlink(nullptr), taillink(nullptr) {}
    uint64_t tailidx; 
    uint64_t headidx;   
    EdgeNode* headlink;
    EdgeNode* taillink;
...
template<typename T>
struct VertexNode {
    VertexNode(const std::shared_ptr<T>& _tempNode) 
        : data(_tempNode), firstin(nullptr), firstout(nullptr), outdegree(0), indegree(0), layer(-1) {}
...
class CrossGraphLink {
    static_assert(std::is_base_of<AbstractNode, T>::value, "T must inherit from AbstractNode");
public:
    ...
    std::string getNodeTypeString(int index)const;
    NodeType getNodeTypeEnum(int index)const;
...
private:
    ...
    //修改node属性
    bool modifyNodeType(const std::string& name, NodeType type);
...
template<typename T>
bool CrossGraphLink<T>::modifyNodeType(const std::string& name, NodeType type) {
        int idx = getVertexIndex(name);
        if (idx == -1) return false;
        // 注意：需要配合节点工厂或者所有类型有统一构造函数
        // 简化为仅支持 AbstractNode 的标准子类
        switch (type) {
            case NodeType::Or:  m_verticesArray[idx].data = std::make_shared<OrNode>(name); break;
            case NodeType::And: m_verticesArray[idx].data = std::make_shared<AndNode>(name); break;
            case NodeType::Not: m_verticesArray[idx].data = std::make_shared<NotNode>(name); break;
            case NodeType::Maj: m_verticesArray[idx].data = std::make_shared<MajNode>(name); break;
            case NodeType::Redundancy: m_verticesArray[idx].data = std::make_shared<RedundancyNode>(name); break;
    ...
    for (int i = 0; i < is_traversed.size(); ++i) {
        if (!is_traversed[i] ) {
            ...
            if(outputVertexIndexVec.find(i) == outputVertexIndexVec.end()){
                if(typeid(*m_verticesArray[i].data) == typeid(RedundancyNode) || typeid(*m_verticesArray[i].data) == typeid(NotNode)){
                    continue;
                }else{
...
template<typename T>
std::string CrossGraphLink<T>::getNodeTypeString(int index) const {
    const auto& v = m_verticesArray.at(index);
    switch (v.data->getNodeType()) {
        ...
        case NodeType::Redundancy: return "wire";
...
template<typename T>
NodeType CrossGraphLink<T>::getNodeTypeEnum(int index) const {
    const auto& v = m_verticesArray.at(index);
    return v.data->getNodeType();
}
