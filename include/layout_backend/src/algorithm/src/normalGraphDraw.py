...
class NormalGraphDraw:
    def __init__(self, v_file_path):
        ...
        self.structural_tuning = {
            "structural_similarity_weight": 2.0,
            "structural_extra_shift_scale": 2.0,
            "structural_base_score": 1.0,
            "structural_max_nodes": 8,
            "structural_max_shift": 8,
            "global_structural_max_nodes": 12,
            "global_structural_max_shift": 6,
    ...
    def set_structural_tuning(self, **kwargs):
        for key, value in kwargs.items():
            if key in self.structural_tuning:
                self.structural_tuning[key] = value
        return dict(self.structural_tuning)

    def _refresh_coord_cache(self):
        coords_cpu = self.coords.tolist()
        node_ids_cpu = self.node_ids.tolist()
        self._node_coord = {}
        self._coord_set = set()
    ...
    def caculate_rough_placement(self):
        """
        返回所有节点的粗略位置, 用NumPy数组存储 (x, y)，整数坐标，不再放置到网格
        """
        coords = []   # 存 (x, y)
        ...
        # 3. 转成 NumPy 整数数组 [N, 2]
        self.node_ids = np.asarray(node_ids, dtype=np.int32)  # [N]
        self.coords   = np.asarray(coords, dtype=np.int32)  # [N, 2]
        self.rough_coords = self.coords.copy()  # 保存初始粗略位置
    ...
    def get_fanin_directions(self):
        ...
        for nid_t in self.node_ids.tolist():
            nid = int(nid_t)
    ...
    def _compact_layout_pass(self, max_iters=64, verbose=False):
        ...
        reductions = 0
        last_legal_coords = self.coords.copy()
        self.contraction_history = []
        self.contraction_evaluations = 0
        self.contraction_global_evaluations = 0
        self.contraction_recursive_evaluations = 0
    ...
    def place_all_nodes_on_chessboard(self):
        """
        遍历 NumPy 数组中的坐标，把节点放置到 mapChessboard
        """
    ...
    def targeted_corridor_expansion(
        ...
        checkpoint_coords = self.coords.copy()
        checkpoint_order = tuple(baseline_order or self._all_route_pairs())
        best_coords = checkpoint_coords.copy()
        ...
        for row_ops, col_ops, label in plans[:max(1, int(max_candidates))]:
            if time.perf_counter() - started >= max(0.1, float(timeout_sec)):
                break
            self.coords = checkpoint_coords.copy()
            self._coord_cache_dirty = True
            self.move_rows_and_cols(row_ops, col_ops)
            # A cut can delay only a subset of the inputs by a whole cycle.
            # Repair launch geometry before trying the same fixed-template router.
            self.align_primary_input_launches()
    ...
    def verify_clock_template_consistency(self, verbose=True):
        ...
        conflicts.extend(self._primary_launch_conflicts())
        if not conflicts:
    ...
    def _primary_launch_conflicts(self):
        """Right/down epochs are x+y, not just (x+y) modulo four.

        All connected primary inputs must start in the same clock cycle.
        Checking this during contraction prevents an otherwise legal row cut
        from introducing a whole-cycle delay at a reconvergent gate.
        """
        self._ensure_coord_cache()
        inputs = sorted(getattr(getattr(self, "parse", None), "getInputNodesIndex", ()))
        inputs = [int(node) for node in inputs if int(node) in self._node_coord]
        if not inputs:
            return []
        first = self._node_coord[inputs[0]]
        cycle = sum(first) // 4
        return [(first, self._node_coord[node], (inputs[0], node), cycle,
                 sum(self._node_coord[node]) // 4)
                for node in inputs[1:] if sum(self._node_coord[node]) // 4 != cycle]

    def align_primary_input_launches(self):
        """Create a common input diagonal before routing on the fixed template.

        A free upper-left margin makes every relocated input monotonically
        reachable from its original fanouts. Existing logic keeps its relative
        geometry; routes are then regenerated by the same right/down router.
        No phase is reassigned outside the coordinate-defined 2DDWave pattern.
        """
        if not self._primary_launch_conflicts():
            return False
        inputs = [int(node) for node in self.parse.getInputNodesIndex
                  if int(node) in self._node_coord]
        inputs.sort(key=lambda node: (self._node_coord[node][0] -
                                      self._node_coord[node][1], node))
        margin = 2 * (len(inputs) - 1)
        self.coords += margin
        indices = {int(node): i for i, node in enumerate(self.node_ids.tolist())}
        for index, node in enumerate(inputs):
            self.coords[indices[node]] = (2 * index, margin - 2 * index)
        self._coord_cache_dirty = True
        self._ensure_coord_cache()
        return True
    ...
    def structural_guided_node_reposition(self, failed_pairs, max_nodes=None, max_shift=None):
        """
        ...
        if max_nodes is None:
            max_nodes = int(self.structural_tuning["structural_max_nodes"])
        if max_shift is None:
            max_shift = int(self.structural_tuning["structural_max_shift"])
        similarity_weight = float(self.structural_tuning["structural_similarity_weight"])
        extra_shift_scale = float(self.structural_tuning["structural_extra_shift_scale"])
        base_score = float(self.structural_tuning["structural_base_score"])
        ...
        for (src, dst), direction in failed_pairs.items():
            ...
            structural_factor = 1.0 - (sim + 1.0) / 2.0

            margin = 1 if direction == (-1, 0) else 0
            required_x = src_x + margin
            violation = max(0, required_x - dst_x)
            score = base_score + violation + similarity_weight * structural_factor
            pressure[dst] += score

            structural_extra = int(round(extra_shift_scale * structural_factor))
            target = required_x + structural_extra
    ...
    def route_until_success(
        ...
        if self.align_primary_input_launches():
            self._refresh_fanin_directions_for_current_coords()
        ...
        best_failed_coords = self.coords.copy()
        ...
        for round_index in range(max_rounds):
            ...
            per_axis_limit = min(
                int(self.structural_tuning["route_insert_max_ops"]),
    ...
    def _route_with_repair(self, verbose=False, repair_iters=2, use_structural_reposition=True):
        _ = use_structural_reposition  # compatibility with legacy callers
    ...
    def one_step_optimization(
        ...
        self.contraction_exhausted = False
        self.contraction_runtime_sec = 0.0
        if structural_tuning:
            self.set_structural_tuning(**structural_tuning)
        ...
        for template_round in range(template_rounds):
            ...
            row_ops, col_ops = self.clock_template_conflicts_to_insert_ops(
                conflicts,
                max_ops_per_iter=int(self.structural_tuning["phase_insert_max_ops"]),
            )
            if not row_ops and not col_ops:
                break
            if verbose:
