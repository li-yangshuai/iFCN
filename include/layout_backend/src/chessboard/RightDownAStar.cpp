...
void RightDownAStar::prepareActiveSourceConstraints(
    const std::pair<int,int>& start)
{
    activeForeignAxes.clear();
    auto addRoute = [&](const std::vector<std::pair<int,int>>& path) {
        if (path.size() < 3 || path.front() == start) return;
        for (size_t i = 1; i + 1 < path.size(); ++i) {
            const auto& previous = path[i - 1];
            const auto& current = path[i];
            const auto& next = path[i + 1];
            unsigned char requiredAxis = 0;
            if (previous.second == current.second && current.second == next.second) {
                requiredAxis = 2;  // cross a horizontal foreign wire vertically
            } else if (previous.first == current.first && current.first == next.first) {
                requiredAxis = 1;
            }
            auto inserted = activeForeignAxes.emplace(current, requiredAxis);
            if (!inserted.second) inserted.first->second &= requiredAxis;
        }
    };
    // The board can already contain routes made by another router instance;
    // finishedRoutes also includes paths not yet saved through the Python API.
    for (const auto& entry : board.nodePairRoutes) addRoute(entry.second);
    for (const auto& entry : finishedRoutes)
        for (const auto& path : entry.second) addRoute(path);
}

bool RightDownAStar::canUseActiveRouteCell(
    const std::pair<int,int>& coord) const
{
    const auto foreign = activeForeignAxes.find(coord);
    return (foreign == activeForeignAxes.end() || foreign->second != 0) &&
        board.canPlaceWire(coord);
}

bool RightDownAStar::canUseActiveTransition(
    const std::pair<int,int>& from,
    const std::pair<int,int>& to) const
{
    const int dx = to.first - from.first;
    const int dy = to.second - from.second;
    const unsigned char axis = dx == 1 && dy == 0 ? 1 : (dx == 0 && dy == 1 ? 2 : 0);
    if (axis == 0) return false;
    for (const auto& coord : {from, to}) {
        const auto foreign = activeForeignAxes.find(coord);
        if (foreign != activeForeignAxes.end() && foreign->second != axis) return false;
    }
    return true;
}
    ...
    bool allowOccupiedTarget) const
{
    ...
    while (current != target) {
        ...
        current = {previous.first + dx, previous.second + dy};
        if (!canUseActiveTransition(previous, current)) return false;
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            const uint32_t fromUp = (
                y > 0 && canUseActiveTransition({coord.first, coord.second - 1}, coord)
                    ? previousCost[static_cast<size_t>(x)] : infinity
            );
            const uint32_t fromLeft = (
                x > 0 && canUseActiveTransition({coord.first - 1, coord.second}, coord)
                    ? currentCost[static_cast<size_t>(x - 1)] : infinity
            );
    ...
    std::vector<std::pair<int,int>>& path) const
{
    ...
    for (const auto& existing : existingRoutes) {
        if (existing.size() < 2 || existing[1] != firstStep) {
            continue;
        }
    ...
    const std::vector<std::pair<int,int>>& path)
{
    ...
    for (size_t i = 1; i < path.size(); ++i) {
        if (!canUseActiveTransition(path[i - 1], path[i])) return {};
    }
    ...
    const std::pair<int,int>& fanin_dir)
{
    ...
    prepareActiveSourceConstraints(start);
    ...
    // Endpoint access cells obey the same ownership/orientation rule as
    // interior wires. Capacity alone permits unrelated nets to turn or
    // overlap in parallel at a gate port, creating an electrical short.
    if (!canUseActiveTransition(start, firstStep) ||
        !canUseActiveTransition(preGoal, goal)) {
        return {};
    }
    ...
    if (firstStepAvailable) {
        ...
        if (trySharedPrefixRoute(start, goal, firstStep, preGoal, sharedPrefixPath)) {
