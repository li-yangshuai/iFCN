...
private:
    ...
    void prepareActiveSourceConstraints(const std::pair<int,int>& start);

    bool canUseActiveRouteCell(const std::pair<int,int>& coord) const;
    bool canUseActiveTransition(const std::pair<int,int>& from,
    ...
    // At a different source's wire, only a straight perpendicular crossing
    // is legal. Required axis: horizontal=1, vertical=2, no legal use=0.
    // Same-source shared prefixes are not treated as unrelated crossings.
    std::unordered_map<std::pair<int,int>, unsigned char, pair_hash> activeForeignAxes;
