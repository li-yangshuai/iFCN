...
namespace iFCN_Lab{
    ...
    public:
        ...
        void parseVerilog(std::string _fileName, bool allowMajority = true);
        ...
        std::string getNodeTypeString(int index) const {return graphLink->getNodeTypeString(index);}
        NodeType getNodeTypeEnum(int index) const      {return graphLink->getNodeTypeEnum(index);}
        ...
        const auto& getOutputNodesIndex()const {return outputNodesIndex;}
        const auto& getInputNodesIndex()const  {return inputNodesIndex;}

        //获取同一层的node index pair,只有执行了这个函数，才能获取这两个容器里面的数据
        ...
        //  2. AIOG的DRC优化
        void optimizeAIOG_DRC(int _limit_input, int _limit_and, int _limit_or, int _limit_not, int _limit_majority, int _limit_redundancy);
        // 3.buffer的优化(redundancyNode)消除
        ...
        //获取算法未处理前，真实的电路node数量
        const auto& getOriginCircuitNodeNum() {return originCircuitNodeNum;}
        const auto& getOriginCircuitEdgeNum() {return originCircuitEdgeNum;}
        
        //获取图的有效边
        std::vector<std::pair<int,int>> getEffectiveEdges()const ;
        const auto& getEffectiveNodes()const{return effectiveNodes;}

        //获取隐藏的节点
        std::vector<uint64_t> hideBufferNodeIndex;
        std::vector<uint64_t> hideNotNodeIndex;
        std::multimap<uint64_t, std::pair<uint64_t, uint64_t>> hide_not_place_pair;
    ...
    private:
        ...
        std::set<unsigned int> inputNodesIndex;
        std::set<unsigned int> effectiveNodes;     
        
        //只有调用optimizeAIOG_DRC函数后，才会更新这个数值
        int originCircuitNodeNum = 0; //原电路节点数
        int originCircuitEdgeNum = 0;  //原电路边数
    };
