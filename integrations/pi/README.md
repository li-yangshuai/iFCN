...
可选的 Pi 工具集成。核心工程的安装、算法与验证方式见 [项目说明](../../README.md)。
...
在已安装 Pi 的环境中执行（外部项目路径替换为实际目录）：

```bash
cd /path/to/iFCN
python3 integrations/pi/install.py \
  "$PWD" /path/to/pi-web-project
```
...
> 使用 /path/to/iFCN/integrations/pi/examples/xor2.v，完成 DAG、门级布局布线、器件版图、Bistable 仿真和标准能耗分析，解释检查结果并给出产物路径。
...
Pi CLI 原生会自动发现项目扩展，LLM 可直接识别工具描述。`.pi/agents/*.md` 的主智能体选择是本工程 Pi Web 提供的能力。CLI 使用同一主智能体提示词时，可把 `agent.md` 去掉 YAML 头部后通过 Pi 的 `--system-prompt` 加载；仅注册工具可 `pi -e /path/to/iFCN/integrations/pi/extension.ts`。
...
默认优先使用 `build-release`，目录不存在时使用 `build`；Python 优先使用本地 `myenv`，不存在时使用当前解释器。Compact/June 还需构建 `ifcn_combinational_pnr` 目标。迁移机器可设置 `IFCN_BUILD_DIR`、`IFCN_PYTHON`、`IFCN_NATIVE_PNR`、`IFCN_RUNS_DIR`；Pi 扩展也支持 `IFCN_ROOT`。capabilities 单独报告每种算法的可用状态，无需把敏感模型配置复制进工程。
...
- 时序适配在 capabilities 中保留，当前明确为 `supported=false`。后续需接入 Yosys/SeqIR、真实反馈 P&R、tile phase/epoch/II 验证与独立物理验证；遵守[时序设计说明](../../docs/sequential-design.md)中 coarse clock tile 的语义。
...
node integrations/pi/smoke.mjs /path/to/pi-web-project
```
