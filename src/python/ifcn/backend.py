...
ALGORITHM = ROOT / 'include/layout_backend/src/algorithm'
ALGORITHMS = ('normal_2ddwave', 'irregular')
sys.path[:0] = [str(ALGORITHM), str(ALGORITHM / 'main')]
os.environ.setdefault('MPLBACKEND', 'Agg')
os.environ.setdefault('IFCN_LAYOUT_PARSE_MODE', 'compact')
...
def run_normal(args, out, summary):
    ...
    options = SimpleNamespace(seed_retries=0,
...
def run_native(args, out, summary):
    ...
    summary['algorithm_parameters'] = {
        'phase_count':4, 'timeout_s':args.budget, 'max_same_phase':4,
        'max_attempts':320, 'seed_control':'deterministic_internal_search',
        'objective':'occupied_tile_area_then_physical_cells_then_route_length',
        'search_budget_s':max(0.1, args.budget-2)}
    candidate_path = out/'layout_candidate.json'
    with (out/'native.log').open('w') as log:
        try:
            result = subprocess.run([str(executable),'irregular',args.input,str(candidate_path),
                                     '--budget',str(max(0.1,args.budget-2)),'--attempts','320'],
    ...
    summary.update(layout_legal=legal, failed_edge_count=len(expected-actual), clock_scheme='irregular',
                   ...
                   run_time_s=candidate.get('run_time_s'), resolved_verilog=args.input,
                   boolean_canonicalization=True, native_search={key:candidate.get(key) for key in ('selected_seed','search_budget_expired','attempts','objective')})
    write_json(out/'pnr.json',summary)
    if not legal:
