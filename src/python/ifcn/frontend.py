"""Strict scalar combinational Verilog front end for the circuit workflow.

Normalize supported Boolean expressions, preserve exact majority identities and
share repeated logic before the native scalar parser performs physical synthesis.
"""
import ast
import graphlib
import re
...
def normalize_verilog(source):
    ...
    temporaries = []
    shared_gates = {}
    optimization = {"majority_gates_preserved": 0, "common_expressions_reused": 0}

    def gate(expr, target=None):
        if target is not None:
            emitted.append((target, expr))
            return target
        if expr in shared_gates:
            optimization["common_expressions_reused"] += 1
            return shared_gates[expr]
        ...
        shared_gates[expr] = candidate
        return candidate

    def majority_literals(node):
        # M(a,b,c) = ab | ac | bc. Only this exact three-pair identity
        # becomes a native majority gate; parentheses alone carry no meaning.
        def terms(item):
            if isinstance(item, ast.BinOp) and isinstance(item.op, ast.BitOr):
                return terms(item.left) + terms(item.right)
            return [item]
        def literal(item):
            if isinstance(item, ast.Name):
                return item.id
            if isinstance(item, ast.UnaryOp) and isinstance(item.op, ast.Invert) and isinstance(item.operand, ast.Name):
                return "~" + item.operand.id
            return None
        products = terms(node)
        if len(products) != 3:
            return None
        ...
        for product in products:
            if not isinstance(product, ast.BinOp) or not isinstance(product.op, ast.BitAnd):
                return None
            left, right = literal(product.left), literal(product.right)
            if left is None or right is None or left == right:
                return None
            pairs.add(tuple(sorted((left, right))))
        names = sorted({name for pair in pairs for name in pair})
        if len(names) != 3 or pairs != {(names[0], names[1]), (names[0], names[2]), (names[1], names[2])}:
            return None
        return names
    ...
    def lower(node, target=None):
        ...
        majority = majority_literals(node)
        if majority:
            optimization["majority_gates_preserved"] += 1
            a, b, c = majority
            return gate(f"({a} & {b}) | ({a} & {c}) | ({b} & {c})", target)
        left, right = lower(node.left), lower(node.right)
        # All supported binary gates are commutative. Canonical temporary
        # expressions share repeated NOT/AND/OR subgraphs without merging
        # separately named primary-output drivers.
    ...
    return normalized, {"module": module, "inputs": declarations["input"], "warnings": warnings,
                        ...
                        "frontend": "scalar-boolean-v2", "optimization": optimization,
                        "functional_signoff": "not_performed"}
