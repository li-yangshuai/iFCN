...
from ifcn.workflow import check_pnr, energy_summary, execute, qca_interface_report, run_pnr
from ifcn import workflow as agent
...
class FrontendTests(unittest.TestCase):
    def test_majority_is_preserved_only_for_the_three_pair_identity(self):
        for expr, expected in [("(a & b) | (a & ~c) | (b & ~c)", 1),
                               ("(a & b) | (a & c) | (a & c)", 0),
                               ("(a | b) & (a | c)", 0)]:
            normalized, meta = normalize_verilog(
                f"module m(input a,b,c, output y); assign y={expr}; endmodule")
            self.assertEqual(meta['optimization']['majority_gates_preserved'], expected)
            if expected:
                self.assertNotIn('ifcn_tmp_', normalized)

    def test_repeated_internal_inverter_and_commuted_and_are_shared(self):
        normalized, meta = normalize_verilog(
            "module m(input a,b,c, output y); assign y=(~a & b) | (b & ~a) | (~a & c); endmodule")
        self.assertEqual(normalized.count('= ~a;'), 1)
        self.assertGreaterEqual(meta['optimization']['common_expressions_reused'], 2)

    def test_separate_primary_output_drivers_are_not_merged(self):
        normalized, _ = normalize_verilog(
            "module m(input a,b, output x,y); assign x=a&b; assign y=b&a; endmodule")
        self.assertIn('assign x = a & b;', normalized)
        self.assertIn('assign y = a & b;', normalized)
...
""")
    ...
    def run_policy(self, algorithm, failure, expect_error=False):
        with tempfile.TemporaryDirectory(prefix="ifcn-policy-test-") as tmp:
            ...
            with patch("ifcn.workflow.execute", backend):
                if expect_error:
    ...
    def test_auto_preserves_failure_and_stops_at_first_valid_result(self):
        ...
        self.assertEqual(calls, ["normal_2ddwave", "irregular"])
        self.assertEqual(report["selected_algorithm"], "irregular")
...
""")
    ...
    def command(self, *args, ok=True):
        run = subprocess.run([sys.executable, str(ROOT / "src/python/ifcn/workflow.py"),
